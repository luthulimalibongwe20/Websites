...
$id = $price = $description = $name = $image = "";
$id_err = $price_err = $description_err = $name_err = $image_err = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Check input errors before updating the record in the database
    if (empty($name_err) && empty($price_err) && empty($description_err)) {
        ...
        if ($stmt = mysqli_prepare($link, $sql)) {
            ...
            mysqli_stmt_bind_param($stmt, "sisi", $param_name, $param_price, $param_description, $param_id);
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                if (mysqli_num_rows($result) == 1) {
                    ...
                    */
                    ...
                    // No matching record found. Redirect to error page or handle the case accordingly.
                }
            } else {
                echo "Oops! Something went wrong. Please try again later.";
            }
...
<?php echo (!empty($description_err)) ? 'is-invalid' : ''; ?>"><?php echo
